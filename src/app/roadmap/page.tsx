    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-14">
          <span className="label-neu text-fn-teal-fonce block mb-5">Roadmap</span>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-14">
          ...
          <div className="space-y-8">
            {phaseDetails.map((phase) => (
              <div key={phase.number} className="card-neu">
                <div className="flex flex-col md:flex-row md:items-center gap-4 mb-8">
                  <span className="font-titre text-4xl font-bold text-fn-teal/40">{phase.number}</span>
                  <div>
                    <h3 className="font-accent font-semibold text-lg text-fn-texte-titre tracking-wide">
                      Phase {phase.number} — {phase.title}
                    </h3>
                    <span className="label-neu">{phase.period}</span>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <span className="label-neu text-fn-teal-fonce block mb-4">Livrables</span>
                    <ul className="space-y-2">
                      {phase.livrables.map((l, i) => (
                        <li key={i} className="flex items-start font-corps text-sm text-fn-texte-sub">
                  ...
                  <div>
                    <span className="label-neu text-fn-teal-fonce block mb-4">Jalons</span>
                    <ul className="space-y-2">
